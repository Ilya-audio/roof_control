-- @description Roof|control: FULLRANGE (Mode Switch)
-- @version 0.95
-- @author Ilya
-- @link https://github.com/Ilya-audio/roof_control
-- @about
--    Part of the Roof|control headphone monitoring system.
--    This script switches the monitoring mode to "Fullrange" emulation via gmem.

local this_mode = 6 -- Индекс для Fullrange
...
local function SyncToolbarNow()
    for mode, hash in pairs(mode_buttons) do
        ...
        if target_id and target_id > 0 then
            -- Включаем текущую, выключаем все остальные
...
-- 5. ИСПОЛНЕНИЕ
-- Передаем режим в JSFX
reaper.gmem_write(3, this_mode)
-- Поднимаем флаг "Ручное управление", чтобы Бубрик пропустил этот цикл обновления
reaper.gmem_write(4, 1) 

-- Отрисовываем изменения в интерфейсе без задержек
SyncToolbarNow()
