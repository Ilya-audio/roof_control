-- @description Roof|control: MAIN (Mode Switch)
-- @version 0.95
-- @author Ilya
-- @link https://github.com/Ilya-audio/roof_control
-- @about
--    Part of the Roof|control headphone monitoring system.
--    This script switches the monitoring mode to "Main" (Reference) via gmem.

local this_mode = 0 -- Индекс для Main
...
local function SyncToolbarNow()
    for mode, hash in pairs(mode_buttons) do
        ...
        if target_id and target_id > 0 then
            -- Включаем текущую (1), выключаем все остальные (0)
...
-- 5. ИСПОЛНЕНИЕ
-- Передаем режим в JSFX (через gmem[3])
reaper.gmem_write(3, this_mode)
-- Поднимаем флаг "Ручное управление", чтобы Бубрик не перезаписывал стейт
