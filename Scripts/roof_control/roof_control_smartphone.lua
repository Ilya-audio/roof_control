-- @description Roof|control: SMARTPHONE (Mode Switch)
-- @version 0.95
-- @author Ilya
-- @link https://github.com/Ilya-audio/roof_control
-- @about
--    Part of the Roof|control headphone monitoring system.
--    This script switches the monitoring mode to "Smartphone" emulation via gmem.

local this_mode = 4 -- Индекс для Smartphone
...
-- 4. Мгновенная синхронизация тулбара
local function SyncToolbarNow()
    for mode, hash in pairs(mode_buttons) do
        ...
        if target_id and target_id > 0 then
            -- Включаем Smartphone (1), остальные гасим (0)
...
-- 5. ИСПОЛНЕНИЕ
-- Шлем режим в память (gmem[3])
reaper.gmem_write(3, this_mode)
-- Флаг для Бубрика: "Не трогай тулбар, я сам" (gmem[4])
