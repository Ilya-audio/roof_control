-- @description Roof|control: SUBWOOFER (Mode Switch)
-- @version 0.95
-- @author Ilya
-- @link https://github.com/Ilya-audio/roof_control
-- @about
--    Part of the Roof|control headphone monitoring system.
--    This script switches the monitoring mode to "Subwoofer" (Low-end check) via gmem.

local this_mode = 1 -- Индекс для Subwoofer
...
-- 4. Мгновенное обновление тулбара
local function SyncToolbarNow()
    for mode, hash in pairs(mode_buttons) do
        ...
        if target_id and target_id > 0 then
            -- Зажигаем Sub (1), гасим остальных (0)
...
-- 5. ИСПОЛНЕНИЕ
-- Пишем в память для JSFX
reaper.gmem_write(3, this_mode)
-- Флаг для блокировки дублирующих обновлений в Бубрике
